"""A-Bogus signing module for Douyin web API requests.

GPL v3 NOTICE
=============
The ``ABogus`` algorithm below is derived from TikTokDownloader by
JoeanAmier (https://github.com/JoeanAmier/TikTokDownloader) and was lifted via
Douyin_TikTok_Download_API by Evil0ctal. The ORIGINAL algorithm is licensed
under the **GNU General Public License v3.0**.

    UPSTREAM_REPO   : https://github.com/Evil0ctal/Douyin_TikTok_Download_API
    ORIGINAL_AUTHOR : https://github.com/JoeanAmier/TikTokDownloader

Because this algorithm is GPL v3, it is kept **isolated** inside the
``server/content/crawlers/signing`` package (see ``__init__.py`` ISOLATION
NOTE). AIFlow is for personal use, which does not trigger GPL source-disclosure
obligations. If AIFlow is ever distributed publicly, this module must be split
into a separate process/service or replaced with a clean-room implementation.

Purpose
-------
Generate the ``a_bogus`` query-parameter signature required by Douyin's web
API endpoints (e.g. ``/aweme/v1/web/aweme/detail/``). The signature is computed
from the canonical query string plus the User-Agent, using SM3 hashing
(``gmssl``) and RC4.

``gmssl`` is an OPTIONAL dependency (declared in the ``remaster`` extras). When
it is not installed, :func:`sign_a_bogus` raises
:class:`~server.content.crawlers.signing.errors.SigningUnavailableError` so the
caller can fall back to yt-dlp.
"""

from __future__ import annotations

from random import choice, randint, random
from re import compile as _re_compile
from time import time
from urllib.parse import quote, urlencode

from .errors import SigningUnavailableError

# ─── Upstream reference ───────────────────────────────────────────────────────

UPSTREAM_REPO: str = "https://github.com/Evil0ctal/Douyin_TikTok_Download_API"

# Pinned upstream commit that the lifted algorithm corresponds to.
UPSTREAM_COMMIT: str = "0b8b8d3"  # crawlers/douyin/web/abogus.py (a_bogus pure-python)

__all__ = ["ABogus", "sign_a_bogus", "UPSTREAM_REPO", "UPSTREAM_COMMIT"]

# Default User-Agent used when callers do not supply one.
_DEFAULT_UA = (
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36"
)


def _require_gmssl():
    """Import ``gmssl`` lazily, raising a friendly error when missing."""
    try:
        from gmssl import func, sm3  # type: ignore[import]
    except ImportError as exc:  # pragma: no cover - exercised via SigningUnavailableError
        raise SigningUnavailableError(
            "A-Bogus signing requires the 'gmssl' package. "
            "Install it with: pip install -e .[remaster]  (or pip install gmssl)"
        ) from exc
    return sm3, func


class ABogus:
    """Pure-python A-Bogus signature generator.

    Ported from the GPL v3 upstream (see module docstring). The public entry
    point is :meth:`get_value`.
    """

    __filter = _re_compile(r"%([0-9A-F]{2})")
    __arguments = [0, 1, 14]
    __end_string = "cus"
    __reg = [
        1937774191,
        1226093241,
        388252375,
        3666478592,
        2842636476,
        372324522,
        3817729613,
        2969243214,
    ]
    __str = {
        "s0": "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",
        "s1": "Dkdpgh4ZKsQB80/Mfvw36XI1R25+WUAlEi7NLboqYTOPuzmFjJnryx9HVGcaStCe=",
        "s2": "Dkdpgh4ZKsQB80/Mfvw36XI1R25-WUAlEi7NLboqYTOPuzmFjJnryx9HVGcaStCe=",
        "s3": "ckdp1h4ZKsUB80/Mfvw36XIgR25+WQAlEi7NLboqYTOPuzmFjJnryx9HVGDaStCe",
        "s4": "Dkdpgh2ZmsQB80/MfvV36XI1R45-WUAlEixNLwoqYTOPuzKFjJnry79HbGcaStCe",
    }
    __browser = (
        "1536|742|1536|864|0|0|0|0|1536|864|1536|864|1536|742|24|24|MacIntel"
    )

    def __init__(self, platform: str | None = None) -> None:
        self.chunk: list[int] = []
        self.size = 0
        self.reg = self.__reg[:]
        # Pre-computed UA char-codes for the default Chrome UA used upstream.
        self.ua_code = [
            76, 98, 15, 131, 97, 245, 224, 133, 122, 199, 241, 166, 79, 34, 90,
            191, 128, 126, 122, 98, 66, 11, 14, 40, 49, 110, 110, 173, 67, 96,
            138, 252,
        ]
        self.browser = (
            self.generate_browser_info(platform) if platform else self.__browser
        )
        self.browser_len = len(self.browser)
        self.browser_code = self.char_code_at(self.browser)

    # ── Random list helpers ────────────────────────────────────────────────
    @classmethod
    def list_1(cls, random_num=None, a=170, b=85, c=45) -> list:
        return cls.random_list(random_num, a, b, 1, 2, 5, c & a)

    @classmethod
    def list_2(cls, random_num=None, a=170, b=85) -> list:
        return cls.random_list(random_num, a, b, 1, 0, 0, 0)

    @classmethod
    def list_3(cls, random_num=None, a=170, b=85) -> list:
        return cls.random_list(random_num, a, b, 1, 0, 5, 0)

    @staticmethod
    def random_list(a=None, b=170, c=85, d=0, e=0, f=0, g=0) -> list:
        r = a or (random() * 10000)
        v = [r, int(r) & 255, int(r) >> 8]
        v.append(v[1] & b | d)
        v.append(v[1] & c | e)
        v.append(v[2] & b | f)
        v.append(v[2] & c | g)
        return v[-4:]

    @staticmethod
    def from_char_code(*args) -> str:
        return "".join(chr(code) for code in args)

    @classmethod
    def generate_string_1(cls, random_num_1=None, random_num_2=None, random_num_3=None) -> str:
        return (
            cls.from_char_code(*cls.list_1(random_num_1))
            + cls.from_char_code(*cls.list_2(random_num_2))
            + cls.from_char_code(*cls.list_3(random_num_3))
        )

    def generate_string_2(self, url_params: str, method="GET", start_time=0, end_time=0) -> str:
        a = self.generate_string_2_list(url_params, method, start_time, end_time)
        e = self.end_check_num(a)
        a.extend(self.browser_code)
        a.append(e)
        return self.rc4_encrypt(self.from_char_code(*a), "y")

    def generate_string_2_list(self, url_params: str, method="GET", start_time=0, end_time=0) -> list:
        start_time = start_time or int(time() * 1000)
        end_time = end_time or (start_time + randint(4, 8))
        params_array = self.generate_params_code(url_params)
        method_array = self.generate_method_code(method)
        return self.list_4(
            (end_time >> 24) & 255,
            params_array[21],
            self.ua_code[23],
            (end_time >> 16) & 255,
            params_array[22],
            self.ua_code[24],
            (end_time >> 8) & 255,
            (end_time >> 0) & 255,
            (start_time >> 24) & 255,
            (start_time >> 16) & 255,
            (start_time >> 8) & 255,
            (start_time >> 0) & 255,
            method_array[21],
            method_array[22],
            int(end_time / 256 / 256 / 256 / 256) >> 0,
            int(start_time / 256 / 256 / 256 / 256) >> 0,
            self.browser_len,
        )

    @staticmethod
    def reg_to_array(a) -> list:
        o = [0] * 32
        for i in range(8):
            c = a[i]
            o[4 * i + 3] = 255 & c
            c >>= 8
            o[4 * i + 2] = 255 & c
            c >>= 8
            o[4 * i + 1] = 255 & c
            c >>= 8
            o[4 * i] = 255 & c
        return o

    def compress(self, a) -> None:
        f = self.generate_f(a)
        i = self.reg[:]
        for o in range(64):
            c = self.de(i[0], 12) + i[4] + self.de(self.pe(o), o)
            c = c & 0xFFFFFFFF
            c = self.de(c, 7)
            s = (c ^ self.de(i[0], 12)) & 0xFFFFFFFF
            u = self.he(o, i[0], i[1], i[2])
            u = (u + i[3] + s + f[o + 68]) & 0xFFFFFFFF
            b = self.ve(o, i[4], i[5], i[6])
            b = (b + i[7] + c + f[o]) & 0xFFFFFFFF
            i[3] = i[2]
            i[2] = self.de(i[1], 9)
            i[1] = i[0]
            i[0] = u
            i[7] = i[6]
            i[6] = self.de(i[5], 19)
            i[5] = i[4]
            i[4] = (b ^ self.de(b, 9) ^ self.de(b, 17)) & 0xFFFFFFFF
        for ll in range(8):
            self.reg[ll] = (self.reg[ll] ^ i[ll]) & 0xFFFFFFFF

    @classmethod
    def generate_f(cls, e) -> list:
        r = [0] * 132
        for t in range(16):
            r[t] = (e[4 * t] << 24) | (e[4 * t + 1] << 16) | (e[4 * t + 2] << 8) | e[4 * t + 3]
            r[t] &= 0xFFFFFFFF
        for n in range(16, 68):
            a = r[n - 16] ^ r[n - 9] ^ cls.de(r[n - 3], 15)
            a = a ^ cls.de(a, 15) ^ cls.de(a, 23)
            r[n] = (a ^ cls.de(r[n - 13], 7) ^ r[n - 6]) & 0xFFFFFFFF
        for n in range(68, 132):
            r[n] = (r[n - 68] ^ r[n - 64]) & 0xFFFFFFFF
        return r

    @staticmethod
    def pad_array(arr, length=60) -> list:
        while len(arr) < length:
            arr.append(0)
        return arr

    def fill(self, length=60) -> None:
        size = 8 * self.size
        self.chunk.append(128)
        self.chunk = self.pad_array(self.chunk, length)
        for i in range(4):
            self.chunk.append((size >> 8 * (3 - i)) & 255)

    @staticmethod
    def list_4(a, b, c, d, e, f, g, h, i, j, k, m, n, o, p, q, r) -> list:
        return [
            44, a, 0, 0, 0, 0, 24, b, n, 0, c, d, 0, 0, 0, 1, 0, 239, e, o, f,
            g, 0, 0, 0, 0, h, 0, 0, 14, i, j, 0, k, m, 3, p, 1, q, 1, r, 0, 0, 0,
        ]

    @staticmethod
    def end_check_num(a: list) -> int:
        r = 0
        for i in a:
            r ^= i
        return r

    @classmethod
    def decode_string(cls, url_string: str) -> str:
        return cls.__filter.sub(cls.replace_func, url_string)

    @staticmethod
    def replace_func(match) -> str:
        return chr(int(match.group(1), 16))

    @staticmethod
    def de(e, r) -> int:
        r %= 32
        return ((e << r) & 0xFFFFFFFF) | (e >> (32 - r))

    @staticmethod
    def pe(e) -> int:
        return 2043430169 if 0 <= e < 16 else 2055708042

    @staticmethod
    def he(e, r, t, n) -> int:
        if 0 <= e < 16:
            return (r ^ t ^ n) & 0xFFFFFFFF
        if 16 <= e < 64:
            return (r & t | r & n | t & n) & 0xFFFFFFFF
        raise ValueError

    @staticmethod
    def ve(e, r, t, n) -> int:
        if 0 <= e < 16:
            return (r ^ t ^ n) & 0xFFFFFFFF
        if 16 <= e < 64:
            return (r & t | ~r & n) & 0xFFFFFFFF
        raise ValueError

    @staticmethod
    def split_array(arr, chunk_size=64) -> list:
        return [arr[i:i + chunk_size] for i in range(0, len(arr), chunk_size)]

    @staticmethod
    def char_code_at(s) -> list:
        return [ord(char) for char in s]

    def write(self, e) -> None:
        self.size = len(e)
        if isinstance(e, str):
            e = self.decode_string(e)
            e = self.char_code_at(e)
        if len(e) <= 64:
            self.chunk = e
        else:
            chunks = self.split_array(e, 64)
            for i in chunks[:-1]:
                self.compress(i)
            self.chunk = chunks[-1]

    def reset(self) -> None:
        self.chunk = []
        self.size = 0
        self.reg = self.__reg[:]

    @classmethod
    def generate_result_unit(cls, n, s) -> str:
        r = ""
        for i, j in zip(range(18, -1, -6), (16515072, 258048, 4032, 63)):
            r += cls.__str[s][(n & j) >> i]
        return r

    @classmethod
    def generate_result(cls, s, e="s4") -> str:
        r = []
        for i in range(0, len(s), 3):
            if i + 2 < len(s):
                n = (ord(s[i]) << 16) | (ord(s[i + 1]) << 8) | ord(s[i + 2])
            elif i + 1 < len(s):
                n = (ord(s[i]) << 16) | (ord(s[i + 1]) << 8)
            else:
                n = ord(s[i]) << 16
            for j, k in zip(range(18, -1, -6), (0xFC0000, 0x03F000, 0x0FC0, 0x3F)):
                if j == 6 and i + 1 >= len(s):
                    break
                if j == 0 and i + 2 >= len(s):
                    break
                r.append(cls.__str[e][(n & k) >> j])
        r.append("=" * ((4 - len(r) % 4) % 4))
        return "".join(r)

    def generate_method_code(self, method: str = "GET") -> list[int]:
        return self.sm3_to_array(self.sm3_to_array(method + self.__end_string))

    def generate_params_code(self, params: str) -> list[int]:
        return self.sm3_to_array(self.sm3_to_array(params + self.__end_string))

    @staticmethod
    def sm3_to_array(data) -> list[int]:
        """SM3-hash *data* and return the digest as a list of byte ints."""
        sm3, func = _require_gmssl()
        b = data.encode("utf-8") if isinstance(data, str) else bytes(data)
        h = sm3.sm3_hash(func.bytes_to_list(b))
        return [int(h[i:i + 2], 16) for i in range(0, len(h), 2)]

    @classmethod
    def generate_browser_info(cls, platform: str = "Win32") -> str:
        inner_width = randint(1280, 1920)
        inner_height = randint(720, 1080)
        outer_width = randint(inner_width, 1920)
        outer_height = randint(inner_height, 1080)
        value_list = [
            inner_width, inner_height, outer_width, outer_height, 0,
            choice((0, 30)), 0, 0, outer_width, outer_height, outer_width,
            outer_height, inner_width, inner_height, 24, 24, platform,
        ]
        return "|".join(str(i) for i in value_list)

    @staticmethod
    def rc4_encrypt(plaintext, key) -> str:
        s = list(range(256))
        j = 0
        for i in range(256):
            j = (j + s[i] + ord(key[i % len(key)])) % 256
            s[i], s[j] = s[j], s[i]
        i = j = 0
        cipher = []
        for k in range(len(plaintext)):
            i = (i + 1) % 256
            j = (j + s[i]) % 256
            s[i], s[j] = s[j], s[i]
            t = (s[i] + s[j]) % 256
            cipher.append(chr(s[t] ^ ord(plaintext[k])))
        return "".join(cipher)

    def get_value(
        self,
        url_params,
        method="GET",
        start_time=0,
        end_time=0,
        random_num_1=None,
        random_num_2=None,
        random_num_3=None,
    ) -> str:
        """Compute the raw (un-quoted) a_bogus value for *url_params*."""
        string_1 = self.generate_string_1(random_num_1, random_num_2, random_num_3)
        params_str = urlencode(url_params) if isinstance(url_params, dict) else url_params
        string_2 = self.generate_string_2(params_str, method, start_time, end_time)
        return self.generate_result(string_1 + string_2, "s4")


# ─── Public API ───────────────────────────────────────────────────────────────


def sign_a_bogus(params: dict, user_agent: str = _DEFAULT_UA) -> str:
    """Compute the URL-encoded ``a_bogus`` signature for a Douyin request.

    Args:
        params:     Query parameters dict (canonical order preserved by caller).
        user_agent: User-Agent string sent with the request. Currently the
                    upstream pure-python ABogus uses a fixed UA char-code table,
                    so this argument is accepted for API compatibility.

    Returns:
        URL-encoded A-Bogus signature string (non-empty).

    Raises:
        SigningUnavailableError: If ``gmssl`` is not installed.
        ValueError: If *params* is not a dict.
    """
    if not isinstance(params, dict):
        raise ValueError("params must be a dict")
    _ = user_agent  # accepted for API symmetry; upstream uses fixed UA table
    value = ABogus().get_value(params)
    return quote(value, safe="")
